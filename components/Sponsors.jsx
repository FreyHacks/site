...
const Sponsors = () => {
	...
	const classParser=(b)=>{
		if(b==="sm"){
			return "w-[45%] sm:w-[25%] xl:w-[20%]"
		}else{
			return "w-[90%] sm:w-[45%] xl:w-[33%]"
		}
	}
  ...
  return (
    <div id='sponsors' className='mt-40 w-full flex flex-col relative '>
			...
			<div className='bg-gradient-to-b from-[#FFD463] to-[#FFE192] w-full min-h-[600px] -mt-1.5 xl:-mt-7'>
					...
					<div className=' flex items-center justify-around flex-wrap gap-x-8 gap-y-8 sm:gap-y-10 px-10 my-10 mx-auto'>
						{data.map((a,n)=><a key={n} href={a.link} target="_blank" className={' z-10 flex flex-col items-center '+classParser(a.size)}><img  className='-z-10 '  src={`svg/${a.svg}`}  width="90%"/></a>)}
					</div>
			</div>
			<div className='w-full'>
